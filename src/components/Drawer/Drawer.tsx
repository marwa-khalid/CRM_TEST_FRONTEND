...
import { PaginationButtonGroup } from "../../components/application/pagination/pagination.tsx";
...
const HistoryActivities: React.FC<DrawerProps> = ({
  visible,
  onClose,
  claimId,
}) => {
  ...
  // State to force DateRangePicker reset
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value) {
      // Reset date picker when user starts typing
  ...
  const handleDateChange = (update: { start?: any; end?: any }) => {
    ...
    // Clear search bar when date is selected
    if (start || end) {
      setSearchTerm("");
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (searchTerm) {
          response = await searchClaimFiles(
            claimIdNum,
            page,
            PAGE_SIZE,
            searchTerm
          ...
          response = await getClaimHistory(claimIdNum, page, PAGE_SIZE);
        }
    ...
    fetchHistory();
  }, [visible, claimId, page, searchTerm, startDate, endDate]);
      ...
      >
        ...
        <div className="flex flex-col h-[calc(100%-80px)] w-full p-6 bg-gray-50/50">
          <div className="mb-4 flex gap-2">
            ...
            <DateRangePicker
              key={datePickerKey} // force reset when key changes
              selectsRange
              startDate={startDate}
              endDate={endDate}
