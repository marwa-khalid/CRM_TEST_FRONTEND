...
export const ProgressBar = ({
    ...
    // ProgressBarBase should take care of rendering the progress visually
    const baseProgressBar = (
      <div
        className={`relative w-full h-2 bg-gray-200 rounded ${className}`} // Base progress bar style
      >
        <div
          className={`h-full rounded ${progressClassName}`} // Progress bar fill style
          style={{ width: `${percentage}%`, transition: "width 0.5s ease-in-out" }} // Set dynamic width
        />
      </div>
    );
      ...
      case "top-floating":
        return (
          <div className="relative flex flex-col items-end gap-2">
            ...
            >
              <div className="text-xs font-weight-600 text-secondary tabular-nums">{formattedValue}</div>
            </div>
          </div>
        );
      case "bottom-floating":
        return (
          <div className="relative flex flex-col items-end gap-2">
            ...
            >
              <div className="text-xs font-weight-600 text-secondary">{formattedValue}</div>
            </div>
          </div>
        );
      default:
        // Fallback or default case, render the base progress bar
