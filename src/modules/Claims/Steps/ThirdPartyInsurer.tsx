...
const ThirdPartyInsurer = ({ formRef }: any) => {
  const claimId = localStorage.getItem("claimId");
  const STORAGE_KEY = `tpiData_${claimId}`;
const inputStyles = `hover:border-neutral-400 focus:border-blue-500 focus:outline-none font-light transition-colors placeholder:font-['Stack_Sans_Headline']`;
  ...
  // --- ACTIONS ---
  const handleSendNotification = () => {
    formik.setFieldValue("abi_1st_notif_date", new Date().toISOString());
    toast.success("ABI 1st Notification generated.");
  };

  const handleSendPaymentPack = () => {
    formik.setFieldValue("payment_pack_date", new Date().toISOString());
  ...
  const handleCompanySelect = (selected: any) => {
    ...
    setShowDropdown(false);
    formik.setFieldValue(
      "third_party_insurer.first_name",
  ...
  const renderInput = (label: string, field: string, placeholder: string) => (
    <div className="flex flex-col gap-2 w-full">
      <label className="text-neutral-700 text-sm font-weight-300 font-light">{label}</label>
      <input
        type="text"
        value={getNestedValue(field)}
        onChange={(e) => formik.setFieldValue(field, e.target.value)}
        placeholder={placeholder}
        className="px-5 py-4 h-[52px] bg-white rounded border border-neutral-200 text-base font-light outline-none focus:border-blue-500"
      />
    </div>
  );
    ...
    return (
      <div ref={pickerRef} className="flex flex-col gap-2 relative w-full">
        ...
        <div
          ...
          className="px-5 py-4 bg-white h-[52px] rounded border border-neutral-200 flex justify-between items-center cursor-pointer"
        >
          <span className={value ? "text-black" : "text-neutral-300"}>
            {value || "Date"}
          </span>
          <img src={Vector6} alt="calendar" className="w-4 h-4" />
        </div>
  ...
  return (
    <div className="MainContent w-full flex flex-col items-start gap-6 py-1 font-['Stack_Sans_Headline']">
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="h-px bg-neutral-100" />
        <div className="flex flex-col gap-2 w-full">
          <label className="text-neutral-700 text-sm">Title</label>
          <Select
            options={[
              { value: "mr", label: "Mr" },
              { value: "mrs", label: "Mrs" },
            ...
            value={[
              { value: "mr", label: "Mr" },
              { value: "mrs", label: "Mrs" },
            ].find((o) => o.value === formik.values.third_party.gender)}
            onChange={(o: any) =>
              formik.setFieldValue("third_party.gender", o.value)
            }
            components={{
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          {renderInput("Email", "third_party.address.email", "Email")}
        </div>
        <div className="grid grid-cols-2 gap-5">
          {renderInput("Telephone", "third_party.address.mobile_tel", "+44")}
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4 shadow-sm">
        ...
        <div className="h-px bg-neutral-100" />
        <div className="flex flex-col gap-2 relative">
          <label className="text-gray-700 text-sm font-weight-300 font-light">
            Company Name
          </label>
          <input
            type="text"
        ...
        <div className="flex flex-col gap-2">
          <label className="text-neutral-700 text-sm font-weight-300 font-light">
            Address
          </label>
          <LeafletAutocompleteMap
            disabled={false}
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          {renderInput("Policy Number", "policy_number", "Enter Policy Number")}
        </div>
        <div
          className="flex items-center gap-2 cursor-pointer"
          onClick={() =>
        ...
        >
          <div
            className={`w-5 h-5 rounded ${formik.values.claim_validation ? "bg-blue-600 border-[6px] border-blue-200" : "bg-neutral-300"}`}
          />
          <span className="text-sm">Client’s Claim in Validation</span>
        </div>
        <div className="flex justify-between items-center pt-2">
          ...
          >
            <div
              className={`w-5 h-5 rounded ${showButton ? "bg-blue-600 border-[6px] border-blue-200" : "bg-neutral-300"}`}
            />
            <span className="text-sm">ABI Insurer?</span>
          </div>
          {showButton && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-blue-100 text-primary rounded text-sm"
            >
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          {renderInput(
            "Tel. Main",
            "third_party_handling.address.mobile_tel",
            "Tel. Main",
          )}
        </div>
        <div className="grid grid-cols-2 gap-5">
          {renderInput(
            "Email Address",
            "third_party_handling.address.email",
            "Email",
          )}
      ...
      <div className="self-stretch p-5 bg-indigo-50 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-5">
          <div className="flex flex-col gap-2">
            ...
            <Select
              ...
              styles={customStyles}
              placeholder="Select Category"
              value={lookups.reasons.find(
                (o: any) => o.id === formik.values.reason_new_mid_id,
              )}
              onChange={(o: any) =>
                formik.setFieldValue("reason_new_mid_id", o.value)
              }
        ...
        >
          <div
            className={`w-5 h-5 rounded ${formik.values.new_mid_search_processed ? "bg-blue-600 border-[6px] border-blue-200" : "bg-neutral-300"}`}
          />
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center bg-gray-50 p-4 rounded-lg">
          ...
          <div className="w-64">
            <span className="text-sm font-weight-400 text-neutral-700">
              ABI 1st Notification Sent On
            </span>
            <p className="text-sm text-neutral-600 font-weight-300 font-light">
              {formatDisplayDate(formik.values.abi_1st_notif_date)}
            </p>
          </div>
        </div>
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4">
          <label className="text-sm font-weight-400">ABI Insurer ?</label>
            ...
            {["Yes", "No"].map((opt) => (
              ...
              >
                <input
                  type="radio"
                  className="hidden"
                  checked={formik.values.abi_insured === opt}
                  onChange={() => formik.setFieldValue("abi_insured", opt)}
                />
                <img
                  src={formik.values.abi_insured === opt ? Yes : No}
                  className="w-5 h-5"
                  alt=""
                />
        ...
        <div className="grid grid-cols-2 gap-5">
          <div className="flex flex-col gap-2">
            ...
            <Select
              ...
              value={lookups.stances.find(
                (o: any) => o.id === formik.values.liability_stance_id,
              )}
              onChange={(o: any) =>
                formik.setFieldValue("liability_stance_id", o.value)
              }
              components={{
                DropdownIndicator: BlueDropdownIndicator,
                IndicatorSeparator: () => null,
      ...
      <div className="self-stretch p-5 rounded-lg border border-neutral-100 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 w-full">
          <label className="text-neutral-700 text-sm">Settlement Status</label>
          <Select
            options={lookups.statuses.map((s: any) => ({
              value: s.id,
              label: s.label,
            ...
            value={lookups.statuses.find(
              (o: any) => o.id === formik.values.settlement_status_id,
            )}
            onChange={(o: any) =>
              formik.setFieldValue("settlement_status_id", o.value)
            }
