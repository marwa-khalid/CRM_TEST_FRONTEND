...
import { sendPayHirer } from "../services/emailService";
...
const FleetPayHirerEmailModal: React.FC<Props> = ({
  open,
  onClose,
  hireId,
  amount,
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const res = await sendPayHirer(hireId, { to: all.join(", "), cc: cc.trim() || undefined, amount, reason });
      if (res.status === "sent") {
        toast.success("Pay hirer email sent.");
        onClose();
    ...
    <div className="fixed inset-0 z-[140] bg-black/40 flex items-center justify-center p-4 font-sans-headline">
      <div className="w-[680px] max-w-full h-[88vh] bg-white rounded-lg flex flex-col overflow-hidden">
        <div className="px-6 py-4 flex justify-between items-center border-b border-neutral-100 shrink-0">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-neutral-100 text-neutral-500 text-xl leading-none">x</button>
        </div>
              ...
              <div className="min-h-[52px] px-3 py-2 bg-white rounded-sm outline outline-1 -outline-offset-1 outline-neutral-200 focus-within:outline-neutral-900 flex flex-wrap items-center gap-2">
                {recipients.map((r, i) => (
                  <span key={`${r}-${i}`} className="flex items-center gap-1 pl-3 pr-1.5 py-1 bg-neutral-100 rounded-full text-neutral-800 text-sm max-w-full">
                    ...
                    <button type="button" onClick={() => removeRecipient(i)} className="text-neutral-400 hover:text-red-500 text-base leading-none shrink-0">x</button>
                  </span>
                ))}
                <input
                  type="text"
...
export default FleetPayHirerEmailModal;
