      ...
      <div className="w-[900px] max-w-full bg-white h-full flex flex-col p-10 gap-5">
        ...
        <div className="flex justify-between items-center">
          <div className="text-black text-2xl font-weight-600 leading-6">
            {loading ? "…" : `${group.count} Vehicles`}
          </div>
          <div className="text-black text-2xl font-weight-600 leading-6">
            {loading ? "" : fmtTotal(group.total)}
          </div>
        </div>

        {/* body: loader while fetching, otherwise the grid of vehicle cards */}
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3">
            <div className="w-8 h-8 rounded-full border-2 border-blue-500 border-t-transparent animate-spin" />
            <div className="text-neutral-400 text-sm">Loading {title.toLowerCase()}…</div>
          </div>
        ) : (
